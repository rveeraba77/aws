// Define Packer Settings for Amazon EC2 plugins
...
// Define Packer variable for Amazon
variable "region" {
  type = string
  description = "Region."
}
...
source "amazon-ebs" "ec2_amazon_ubuntu_2024_inst" {
  ami_name = "sloopstash-amazon-ubuntu-2024-04-v1.1.1-ami"
  region = var.region
  ...
  ssh_timeout = "1m"
  tags = {
    Name = "sloopstash-amazon-ubuntu-2024-04-v1.1.1-ami"
    Region = var.region
...
build {
  name = "ec2_amazon_ubuntu_2024_ami"
  sources = ["source.amazon-ebs.ec2_amazon_ubuntu_2024_inst"]
  provisioner "shell" {
    only = ["amazon-ebs.ec2_amazon_ubuntu_2024_inst"]
